...
const getNEARPrice = async () => {
	try {
		...
		if (import.meta.env.PROD) {
			api = 'https://europe-west3-near-for-notion.cloudfunctions.net/queryCoinMarketCap'
		}
	...
	try {
		...
		if (account_id) {
			if (account_id.indexOf('.near') === -1) {
				...
				console.log('BEOFRE Connect config2', Object.assign({ deps: { keyStore: keyStore.value } }, config.value))
				...
				balance.value = nearAPI.utils.format.formatNearAmount((await account.value.getAccountBalance())?.available || '0').slice(0, 5)
			}
		}
	} catch (e) {
		err_msg.value = 'Provide an existing NEAR account_id\n' +
			`"${account_id}" is not valid\n`
